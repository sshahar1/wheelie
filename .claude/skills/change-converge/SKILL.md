...
5. **Post-hook override**: After running all `after_converge` hooks, if any mandatory hook:
   ...
   **Loop routing signal**: The caller should check for `next-spec.md` in the change directory:
   - If `next-spec.md` exists → the spec itself needs correction; route to `/spec-specify` (feeding next-spec.md as input), then plan → tasks → implement → converge
   - If `next-spec.md` does not exist but `tasks_appended` → only implementation tasks were appended; route to `/spec-implement` → converge
